...
{
    public array $coreModules = [];
    public array $notCoreModules = ['faq', 'newsletter', 'comments', 'calendar', 'news'];
    public array $deletedIndexData = [];
     ...
     * load tables
     */
    public function initProSearch(): void
    {
        // set Vendor Modules
        if ($this->checkPermission($GLOBALS['PS_SEARCHABLE_MODULES']) && is_array($GLOBALS['PS_SEARCHABLE_MODULES']) && !empty($GLOBALS['PS_SEARCHABLE_MODULES'])) {
    ...
    public function setCoreModules(): void
    {
        ...
        foreach ($this->modules as $k => $module) {
            ...
            foreach ($module['tables'] as $table) {
                $this->coreModules[$table] = $label[0] ?? '[no-label-found]';
            }
        }
    }
    ...
    public function deleteModulesFromIndex()
    {
        ...
        $activeModules = StringUtil::deserialize(Config::get('searchIndexModules')) ? StringUtil::deserialize(Config::get('searchIndexModules')) : array();
    ...
    public function sendDataToIndex($dc, $value = '')
    {
        ...
        if ($act == 'cut') {
            ...
            $dcaArr = Database::getInstance()->prepare('SELECT * FROM ' . $tablename . ' WHERE ' . $colname . ' = ?')->execute($id)->row();
        }
    ...
    public function setSearchContent($db, $doTable)
    {
        ...
        foreach ($colsSearchContent as $content) {
            ...
            $ct = StringUtil::deserialize($db[$content]);
        ...
        return mb_strtolower($strContent);
    ...
    public function clearSearchIndexTable($dca): void
    {
        if (!$dca) {
            return;
        }
    ...
    public function saveSingleIndexIntoDB($data, $dca): void
    {
    ...
    public function deleteDataFromIndex($dc): void
    {
